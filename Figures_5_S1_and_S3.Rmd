...
Figure S3 constants for heatmap. **Only run to create *Figure S3* heatmap**
```{r figS3_constants}
DATA <- cmn_dsq_df
TAXA <- cmn_dsq_df
ROOT <- "ASV_31"
...
Set up colors for row annotations. 
```{r annotation_colors}
set.seed(17)
...
row_colors <- list(`Bee species` = c("cerana" = "#8cb369", "mellifera" = "#f4e285"),
                ...
                `Sample type` = c("mouth" = "#bc4b51", "crop" = "#f4a259", "nectar" = "#0ED8DB"))
```

Plot heatmap figure. 
```{r heatmap}
# Figure 5
# fun_args <- list(mat = COMMUNITY, cluster_cols = as.hclust.phylo(PHYLOGENY), cluster_rows = SAMPLE_CLUSTERS, annot_legend = F, annotation_row = row_annotations, annotation_col = NA, labels_row = NULL, labels_col = genus_annotations$Genus, fontsize_row = 11, fontsize_col = 9, angle_col = 45)
...
cmn_HM = pheatmap(mat = fun_args$mat, color = heatmap_coloring,
                  cluster_cols = fun_args$cluster_cols, cluster_rows = fun_args$cluster_rows, 
                  border_color = "transparent",
                  legend = F, annotation_legend = fun_args$annot_legend,
                  legend_breaks = leg_break,
                  legend_labels = leg_labels, 
                  annotation_row = fun_args$annotation_row, annotation_col = fun_args$annotation_col, 
...
```{r fig5_tinkering}
cmn_HM$gtable$grobs[[6]]$x[1:39] <- unit(1, "npc") + unit(rep(-11, 39), "bigpts")
cmn_HM$gtable$grobs[[6]]$x[40:78] <- unit(1, "npc") + unit(rep(-31, 39), "bigpts")
cmn_HM$gtable$grobs[[6]]$x[79:117] <- unit(1, "npc") + unit(rep(-51, 39), "bigpts")
cmn_HM$gtable$grobs[[6]]$width <- unit(18, "bigpts")
cmn_HM$gtable$grobs[[7]]$x <- unit(1, "npc") + unit(c(-13, -33, -53), "bigpts")
cmn_HM$gtable$widths[2] <- unit(61, "bigpts")
cmn_HM$gtable <- gtable_add_cols(cmn_HM$gtable, widths = unit(40, "bigpts"), pos = 0)
```
...
grid.arrange(textGrob("A", gp = gpar(fontsize = 13)), textGrob("B", gp = gpar(fontsize = 13), just = c(0.5, 0.5)), 
             textGrob("C", gp = gpar(fontsize = 13)), textGrob("D", gp = gpar(fontsize = 13)), 
             textGrob("winter nectar vs. winter mouth", gp = gpar(fontsize = 11), just = c(0.5,0.3)), 
             textGrob("winter nectar vs. winter crop", gp = gpar(fontsize = 11), just = c(0.5, 0.35)), 
             ...
             dsq_legend,
             layout_matrix = rbind(c(NA, NA, NA, 1, 7, NA, 2, 5, NA, 4, 8), c(NA, NA, 11, 11, 11, 9, 9, 9, 12, 12, 12), 
                                   c(NA, NA, 11, 11, 11, 9, 9, 9, 12, 12, 12), c(NA, NA, 11, 11, 11, NA, 3, 6, 12, 12, 12), 
                                   c(NA, NA, 11, 11, 11, NA, 10, 10, 12, 12, 12), c(NA, 13, 11, 11, 11, NA, 10, 10, 12, 12, 12), 
