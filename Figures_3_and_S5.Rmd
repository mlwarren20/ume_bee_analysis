...
```{r fig3}
# Save 8 x 4 inches
fig3_plot = grid.arrange(alpha_sim$crop$Shannon$alpha_plot + ylim(-0.2, 4) + theme_classic() + 
...
Jpeg figure 3 created using the following code: 
```{r fig3_jpg}
top.vp = viewport(layout = grid.layout(3, 3, widths = unit(c(26, 160, 26), "mm"), heights = unit(c(26, 180, 26), "mm")))
plot.vp = viewport(layout.pos.col = 2, layout.pos.row = 2, name = "plot")
splot = vpTree(top.vp, vpList(plot.vp))

jpeg(filename = "Fig3.jpg", width = 238, height = 227, units = "mm", pointsize = 16, res = 400)
pushViewport(splot)
seekViewport("plot")
grid.draw(fig3_plot)
draw(samType_leg, x = unit(0.25, "npc"), y = unit(0.27, "npc"), just = c("right", "bottom"))
draw(species_shape_legend, x = unit(0.27, "npc"), y = unit(0.17, "npc"), just = c("right", "bottom"))
grid.lines(x = unit(c(0.1, 0.35), "npc"), y = unit(1, "npc"), gp = gpar(lty = "solid", lwd = 2))
grid.lines(x = unit(c(0.38, 0.97), "npc"), y = unit(1, "npc"), gp = gpar(lty = "solid", lwd = 2))
grid.text(label = "Summer", x = unit(0.23, "npc"), y = unit(1.02, "npc"), gp = gpar(fontsize = 12))
grid.text(label = "Winter", x = unit(0.68, "npc"), y = unit(1.02, "npc"), gp = gpar(fontsize = 12))
popViewport()
dev.off()
```
