...
Figures 6, S1, and S3 are based on the results of the DESeq2 analysis. So we begin with this analysis. To do a pairwise analysis of each sample type in each season, we started with a list where the sample type (i.e. "crop", "mouth", or "nectar") will be excluded and the season will be included in the data subset.
...
For Figure 6, we only visualized differentially abundant taxa that wer present in at least 50% of the different groups. Since there were 15 crop and mouth groups, we kept taxa present in at least 7 or more of these groups, and for the 9 nectar groups we kept taxa present in at least 4 or more groups. 
```{r 50perc_tax}
...
Figure 6 constants for heatmap. **Only run to create *Figure 5* heatmap**
```{r fig6_constants}
...
Jpeg of figure 6 was created with the following code: 
```{r fig6_jpg}
jpeg(filename = "Fig6.jpg", width = 500, height = 350, units = "mm", res = 400, pointsize = 16)
par(mai = c(1, 1, 1, 1))
