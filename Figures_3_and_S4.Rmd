...
This script produces the alpha diversity Figures 3 and S4 in the manuscript. We begin by clustering ASVs to OTU at 97% identity to make sure we see the same trends even when we allow for more variability. Here we used the methods described in <https://github.com/benjjneb/dada2/issues/947>
```{r OTU_clusters}
...
We made a data frame that included sample metadata as well as various alpha diversity estimates. Since we sampled the Nishi-honjo site twice, we labeled the summer sampling as Nishi-honjo_S site and the winter sampling as Nishi-honjo_W site. 
```{r alpha_div}
alpha_measures = c("Shannon", "Chao1", "Observed")
...
origin_shapes = c("crop" = 21, "mouth" = 23, "nectar" = 22)
...
pseudo_summer_nectar = data.frame(origin = rep("nectar", 4), sample_name = rep(NA, 4), date = rep(NA, 4), 
                                  ...
                                  Observed = rep(NA, 4), Chao1 = rep(NA, 4), se.chao1 = rep(NA, 4), 
                                  Shannon = rep(NA, 4), site2 = c("Ichiigawa", "Kiyokawa 1", "Oshine 1", "Nishi-Honjo_S"), 
...
```{r fig3}
# Save 3.5 x 8 inches
grid.arrange(alpha_sim$crop$Shannon$alpha_plot + xlim(-0.2, 4) + theme_classic() + 
               theme(axis.title.x = element_blank(), legend.position = "none"), 
             alpha_sim$mouth$Shannon$alpha_plot + xlim(-0.2, 4) + theme_classic() + 
               theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), 
                     axis.title.y = element_blank(), legend.position = "none"), 
             alpha_sim$nectar$Shannon$alpha_plot + xlim(-0.2, 4) + theme_classic() + 
               theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), 
                     axis.title = element_blank(), legend.position = "none"), 
             layout_matrix = rbind(c(1, 2, 3), c(1, 2, 3), c(NA, 2, NA)), 
             heights = c(0.255, 0.65, 0.05), 
             widths = c(0.42, 0.28, 0.28))
```
